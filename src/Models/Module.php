...
class Module extends Model
{
    ...
    public static function generateBase($module_name, $icon)
    {
        ...
        if (file_exists(base_path('app/Http/Controllers/'.($names->controller).'.php'))) {
            if (($names->model == 'User' || $names->model == 'Role' || $names->model == 'Permission') && file_exists(base_path('app/'.($names->model).'.php'))) {
                $is_gen = true;
            } elseif (file_exists(base_path('app/Models/'.($names->model).'.php'))) {
                $is_gen = true;
            }
        }
    ...
    public static function generate($module_name, $module_name_db, $view_col, $faIcon, $fields)
    {
        ...
        } elseif (!self::validate_view_column($fields, $view_col)) {
        ...
        switch ($field->field_type) {
            ...
            case 'Multiselect':
                ...
                if (is_array($field->defaultvalue)) {
                    ...
                    $var->default($field->defaultvalue);
                } elseif (is_string($field->defaultvalue) && starts_with($field->defaultvalue, '[')) {
                    $var->default($field->defaultvalue);
                ...
                switch ($ftypes[$field['field_type']]) {
                    ...
                    case 'Dropdown':
                        if ($request->{$field['colname']} == 0) {
                            ...
                            } elseif (starts_with($field['popup_vals'], '[')) {
