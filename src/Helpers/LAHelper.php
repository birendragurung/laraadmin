...
class LAHelper
{
    ...
    public static function print_menu($menu, $active = false)
    {
        ...
        $str = '<li'.$treeview.' '.$active_str.'><a href="'.url(config('laraadmin.adminRoute').'/'.$menu->url).'"><i class="fa '.$menu->icon.'"></i> <span>'.self::real_module_name($menu->name).'</span> '.$subviewSign.'</a>';
